...
//if ($user_check!=''){
    ...
    if($flag == 'shuttleScan'){
        $findOrderSQL = "SELECT ordId, orderid, pickPointEmp, Pick, PickTime, PickBy, DP1, DP1Time, DP1By, Shtl, ShtlTime, ShtlBy,retcp1, retcp1Time, retcp1By FROM `tbl_order_details` WHERE barcode = SUBSTRING('$orderid',1,11) and Shtl is null";
                ...
                if($findOrderRow['pickPointEmp'] != ''){
                    if($findOrderRow['Pick'] != 'Y'){
                        ...
                        if(!mysqli_query($conn, $updateOrdersSQL)){
                            ...
                            $merOrderRefRow = mysqli_fetch_array(mysqli_query($conn, "select merchantCode, merOrderRef,demo from tbl_order_details where orderid='$scannedOrder'"));
...
if($flag == 'retCPScan'){
    $findOrderSQL = "SELECT ordId, orderid, Ret, partial, DropDP2, bank, retcp1, retcp1Time, retcp1By FROM `tbl_order_details` WHERE barcode = SUBSTRING('$orderid',1,11) and retcp1 is null";
    $findOrderResult = mysqli_query($conn, $findOrderSQL);
    if(mysqli_num_rows($findOrderResult) > 0){
        $findOrderRow = mysqli_fetch_array($findOrderResult);
        $scannedOrder = $findOrderRow['orderid'];
...
if($flag == 'dp2Scan'){
    $findOrderSQL = "SELECT ordId, orderid, Shtl, cp2Shuttle, DP2 FROM `tbl_order_details` WHERE barcode = SUBSTRING('$orderid',1,11) and DP2 is null";
    $findOrderResult = mysqli_query($conn, $findOrderSQL);
    if(mysqli_num_rows($findOrderResult) > 0){
        $findOrderRow = mysqli_fetch_array($findOrderResult);
        $scannedOrder = $findOrderRow['orderid'];
...
if($flag == 'dp2pickScan'){
    $findOrderSQL = "SELECT ordId, orderid, dropPointEmp, DP2, PickDrop FROM `tbl_order_details` WHERE barcode = SUBSTRING('$orderid',1,11) and PickDrop is null";
    $findOrderResult = mysqli_query($conn, $findOrderSQL);
    if(mysqli_num_rows($findOrderResult) > 0){
        $findOrderRow = mysqli_fetch_array($findOrderResult);
        $scannedOrder = $findOrderRow['orderid'];
...
//left join (SELECT * from tbl_sla_missed where DATE_FORMAT(creationDate, '%Y-%m-%d') = '$startDate') as slaMissed on tbl_point_info.pointCode = slaMissed.dropPointCode 
    ...
    $pointSumResult = mysqli_query($conn, "select pointid, pointCode, pointName, region, regionSort, shtlOrders.shuttle, dp2pick.pickDate, dp2pick.pick, slaMissed.slaCnt, employeeCount.empCount, cashOrders.cash, retOrders.ret, partialOrders.partial, onHoldOrders.onHold, onHoldSum.onHoldPending, pendingOrders.pending from tbl_point_info 
        left join (SELECT dropPointCode, count(1) as slaCnt FROM `tbl_order_details` left join tbl_point_info on tbl_order_details.dropPointCode = tbl_point_info.pointCode WHERE Cash is null and Ret is null and partial is null and close is null and Shtl='Y' and  orderDate < (DATE_SUB(curdate(), INTERVAL (select sla from tbl_point_info where tbl_point_info.pointCode = dropPointCode ) DAY)) group by dropPointCode) as slaMissed on tbl_point_info.pointCode = slaMissed.dropPointCode 
...
echo '<thead><tr><th colspan=5></th><th colspan=3 style="text-align: center; background-color: #F5F5F5">Status at '.$statusDate.'</th><th colspan=7></th></tr></thead>';
echo '<thead><tr><th>Point Code</th><th>Point Name</th><th>Pick Date</th><th>Shuttled Orders</th><th>Picked Orders</th><th style="text-align: center; background-color: #F5F5F5">Without Status</th><th style="text-align: center; background-color: #F5F5F5">on Hold Pendings</th><th style="text-align: center; background-color: #F5F5F5">Opening Balance</th><th style="color: red">SLA Missed</th><th>Capacity</th><th>Cash</th><th>Ret</th><th>Partial</th><th>on Hold</th><th>% Returned</th></tr></thead><tbody>';
...
if($pointManager == '0'){
    foreach($pointSumResult as $pointSumRow){
        ...
        $openingBalance = $pointSumRow['cash'] + $pointSumRow['ret'] + $pointSumRow['partial'];
        ...
        foreach($pointListResult as $pointListRow){
            ...
            if($pointRegion != $pointSumRow['region']){
                if($regionOrderCount != 0){
                    ...
                    echo '<tr style="font-weight: 800"><td colspan = 3>Total : '.$regionOrderCount.'</td><td>'.$shuttleCount.'</td><td>'.$dp2PickCount.'</td><td style="text-align: center; background-color: #F5F5F5">'.$withoutStatusCount.'</td><td style="text-align: center; background-color: #F5F5F5">'.$onHoldCount.'</td><td style="text-align: center; background-color: #F5F5F5">'.$openingCount.'</td><td>'.$slaMissedCount.'</td><td>'.$executiveCount.'</td><td>'.$cashCount.'</td><td>'.$retCount.'</td><td>'.$partialCount.'</td><td>'.$curOnHoldCount.'</td><td style="text-align: center">'.$deliveryPercent.'</td></tr>';    
                }
                $regionOrderCount = 0;
                $shuttleCount = 0;
                $dp2PickCount = 0;
...
if($flag == 'pointManagerPerfMerchant'){
    ...
    $pointSumResult = mysqli_query($conn, "select pointid, pointCode, pointName, region, regionSort, shtlOrders.shuttle, dp2pick.pickDate, dp2pick.pick, slaMissed.slaCnt, employeeCount.empCount, cashOrders.cash, retOrders.ret, partialOrders.partial, onHoldOrders.onHold, onHoldSum.onHoldPending, pendingOrders.pending from tbl_point_info 
        left join (SELECT dropPointCode, count(1) as slaCnt FROM `tbl_order_details` left join tbl_point_info on tbl_order_details.dropPointCode = tbl_point_info.pointCode WHERE Cash is null and Ret is null and partial is null and close is null and Shtl='Y' and  orderDate < (DATE_SUB(curdate(), INTERVAL (select sla from tbl_point_info where tbl_point_info.pointCode = dropPointCode ) DAY)) and merchantCode = '$merchant' group by dropPointCode) as slaMissed on tbl_point_info.pointCode = slaMissed.dropPointCode 
...
if($flag == 'returnOrdersTrack'){
 ...
 if($flag == 'directDeliveryaccept'){

 $comments = $_POST['reason'];
 $merOrderRef = $_POST['merorderid'];
    ...
    if(mysqli_num_rows($barcodeInfoResult) > 0){
     ...
     if($flag == 'validateBarcode2'){
                ...
                if ($conn->query($sql) === TRUE) {
                  ...
                  if($result ==1)
                  {
                       ...
                       echo '<script>';
                        ...
                        echo '$("#barcodeonscreen").html("'.$barcodeInfoRow['barcodeNumber'].'");';
                      echo '</script>';
             ...
             if(isset($_POST['flag'])){
               ...
               $sql = "INSERT INTO barcode_factory (barcodeNumber, merchant_code, updated_by,state,scanned_by,scanned_at)
               VALUES ('$barcode', '$merchant', '$emp',1,'$user_check',NOW()+INTERVAL 6 HOUR)";
             ...
             if($flag == 'update_barcode_factory'){
                 ...
                 if($table==1)
                 {
                   $barcodeInfoResult =  mysqli_query($conn, "UPDATE barcode_factory SET state=1,scanned_at=NOW()+INTERVAL 6 HOUR,scanned_by='$user_check' WHERE substring(barcodeNumber, 1, 11) = '$orderid'");

                 $totalresult =  mysqli_query($conn, "SELECT updated_by,count(id) as total from barcode_factory where state=1 AND merchant_code ='$merchant_code' AND accepted!='Y' group by updated_by");
                 $sumup =  mysqli_query($conn, "SELECT count(id) as sum from barcode_factory where state=1 AND merchant_code='$merchant_code' AND accepted!='Y'");
                 ...
                 elseif($table==2)
                 {
                     $barcodeInfoResult =  mysqli_query($conn, "UPDATE tbl_barcode_factory_fulfillment SET state=1,scanned_at=NOW()+INTERVAL 6 HOUR,scanned_by='$user_check' WHERE substring(barcodeNumber, 1, 11) = '$orderid'");

                 $totalresult =  mysqli_query($conn, "SELECT updated_by,count(id) as total from tbl_barcode_factory_fulfillment where state=1 AND merchant_id ='$merchant_code' AND state<>2 group by updated_by");
                 $sumup =  mysqli_query($conn, "SELECT count(id) as sum from tbl_barcode_factory_fulfillment where state=1 AND merchant_id='$merchant_code' AND state<>2");
                   if(mysqli_num_rows($totalresult) > 0){
                   $sums = mysqli_fetch_array($sumup);
                   while($row = mysqli_fetch_array($totalresult)){
                      $fields[] = $row;
...
if($flag == 'searchCustMobileNo'){
    ...
    if($orderCount > 1){
        echo 'Error: More than one order found';
        exit();
    }
    foreach($searchOrderResult as $searchOrderRow){
        $custphone = substr($searchOrderRow['custphone'], -$mobileNoLength);
        $custAddress = preg_replace('/[^A-Za-z0-9\. -]/', '', $searchOrderRow['custaddress']);
        if($custphone == $mobileNo){
            echo '<script>';
            echo '$("#searchedMerchant").html("'.$searchOrderRow['merchantName'].'"); ';
            echo '$("#customerName").html("'.$searchOrderRow['custname'].'"); ';
            echo '$("#customerAddress").val("'.$custAddress.'"); ';
            echo '$("#customerPhone").html("'.$searchOrderRow['custphone'].'"); ';
            echo '$("#productPrice").val("'.$searchOrderRow['packagePrice'].'"); ';
            echo '$("#productSizeWeight").val("'.$searchOrderRow['productSizeWeight'].'"); ';
            echo '$("#customerDistrict").select2("val", "'.$searchOrderRow['customerDistrict'].'"); ';
            ...
            echo 'fetch_customerThana_forPick('.$searchOrderRow['customerThana'].')';
            echo '</script>';
            $foundFlag = 'T';
            exit();
        }
...
if($flag == 'acceptOrder'){
    ...
    if(empty($empCode)){
        ...
        if(!mysqli_query($conn, $updateSQL)){
            ...
            $updateSQL1 = "update barcode_factory set accepted = 'Y' where barcodeNumber='$barCode2'";
            mysqli_query($conn, $updateSQL1);
            echo $paperflyID.' accepted for shuttle';
        }   
    }
}
if($flag == 'updateAndAccept'){
    $customerAddress = $_POST['customerAddress'];
    $productPrice = $_POST['productPrice'];
    $packageOptionReq = $_POST['packageOption'];
    $customerThana = $_POST['customerThana'];
    $barcodeVal = substr($_POST['barcodeVal'],0,11);
    $pickedBy = $_POST['pickedBy'];
    ...
    $orderDetailRow = mysqli_fetch_array(mysqli_query($conn, "select * from tbl_order_details where orderid = '$orderid'"));
    $ordId = $orderDetailRow['ordId'];
    $pickPointCode = $orderDetailRow['pickPointCode'];
    $merchantCode = $orderDetailRow['merchantCode'];
    $packageOption = $packageOptionReq;
    $deliveryOption = $orderDetailRow['deliveryOption'];

    $pickPointDistrictRow = mysqli_fetch_array(mysqli_query($conn, "SELECT distinct districtId FROM tbl_point_coverage WHERE pointCode = '$pickPointCode'"));
    $pickUpDistrictID = $pickPointDistrictRow['districtId'];

    $dropPointDistrictRow = mysqli_fetch_array(mysqli_query($conn, "select * from tbl_thana_info where thanaId = '$customerThana'"));
    $dropDistrictID = $dropPointDistrictRow['districtId'];

    $dropPointCodeRow = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM tbl_point_coverage WHERE thanaId = '$customerThana'"));
    $dropPointCode = $dropPointCodeRow['pointCode'];
    ...
    $orderChargeSQL = "SELECT * FROM tbl_rate_type where ratechartId = '$merRateChartId' and packageOption = '$packageOption' and deliveryOption = '$deliveryOption' and destination = '$destination'";
    ...
    $updateOrderSQL = "update tbl_order_details set orderid = '$newOrderID', barcode = '$barcodeVal', pickPointEmp = '$empCode', productSizeWeight = '$packageOption', Pick = 'Y', PickTime = NOW() + INTERVAL 6 HOUR, PickBy = '$pickedBy', dropPointCode = '$dropPointCode', custaddress = '$customerAddress', customerThana = '$customerThana', customerDistrict = '$dropDistrictID', packagePrice = '$productPrice', destination = '$destination', charge = '$charge', cod = '$mercod' where ordId = '$ordId'";
    if(!mysqli_query($conn, $updateOrderSQL)){
        echo 'Error: unable to update order information';
    } else {
        echo $newOrderID;
    }
}
if($flag == 'searchMerchantRef'){
    $merchantCode = $_POST['merchantCode'];
                //$mobileNo = substr($orderid, -3);
    $merRef = $orderid;
    //$mobileNoLength = strlen($mobileNo);
    $foundFlag = 'F';
    $searchOrderResult = mysqli_query($conn, "select tbl_order_details.orderid, tbl_order_details.merOrderRef, tbl_order_details.merchantCode, tbl_merchant_info.merchantName, custname, custaddress, custphone, packagePrice, tbl_thana_info.thanaName, customerThana, customerDistrict, tbl_district_info.districtName from tbl_order_details left join tbl_merchant_info on tbl_order_details.merchantCode = tbl_merchant_info.merchantCode left join tbl_thana_info on tbl_order_details.customerThana = tbl_thana_info.thanaId left join tbl_district_info on tbl_order_details.customerDistrict = tbl_district_info.districtId where tbl_order_details.merchantCode = '$merchantCode' and tbl_order_details.Pick is null");

    foreach($searchOrderResult as $searchOrderRow){
        $merchantRef = $searchOrderRow['merOrderRef'];
        if($merchantRef == trim($merRef)){
            $custAddress = preg_replace('/[^A-Za-z0-9\. -]/', '', $searchOrderRow['custaddress']);
            echo '<script>';
            echo '$("#searchedMerchant").html("'.$searchOrderRow['merchantName'].'"); ';
            echo '$("#customerName").html("'.$searchOrderRow['custname'].'"); ';
...
if($flag == 'scan_report'){
   ...
   $sql = "SELECT count(*) as total FROM `barcode_factory` WHERE scanned_at BETWEEN '$from' AND '$to'";
    $result = mysqli_query($conn, $sql) or die("Error in Selecting " . mysqli_error($conn));
    $barcodeInfoRow = mysqli_fetch_assoc($result);
    //echo $barcodeInfoRow['total'];
    echo '<script>';
                        echo '$("#picked").html("'.$barcodeInfoRow['total'].'"); ';
                        echo '</script>';
