...
if(isset($_POST['data'])){
    ...
    if ($flag == 'Pick'){
        ...
        if (!mysqli_query($conn, $picupdatesql)){
            ...
            if($merchantCode == 'M-1-0484' AND substr($merOrderRef, 0, 2 ) == "RS" ){
                $curl = curl_init();
...
if ($flag == 'Cash'){
    ...
    if($CashAmt != $cashOrgAmount){
        if(!$cashComment){
            ...
            if($merchantCode == 'M-1-0484' AND substr($merOrderRef1, 0, 2 ) == "RS"){
                $curl = curl_init();
...
if ($flag == 'Ret'){
    ...
    $merOrderRefRow = mysqli_fetch_array(mysqli_query($conn, "select merchantCode, merOrderRef, ratechartId from tbl_order_details where orderid='$orderid'"));
